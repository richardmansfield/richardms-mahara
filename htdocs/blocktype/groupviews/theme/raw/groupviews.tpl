    ...
    {foreach from=$groupviews item=view}
        <tr class="{cycle values='r0,r1'}">
            <td>
                ...
                {/if}
                <h3><a href="{$WWWROOT}{$view.url}">{$view.title}</a></h3>
        ...
        <tr class="{cycle values='r0,r1'}">
            <td>
                ...
                {if $view.sharedby}
                    ...
                    {if $view.group}
                        <a href="{$WWWROOT}{group_homepage_url($view.groupdata)}">{$view.sharedby}</a>
                    {elseif $view.owner}
                        <a href="{$WWWROOT}{profile_url($view.user)}">{$view.sharedby}</a>
                    {else}
                        {$view.sharedby}
                    {/if}
                    </span>
...
{if $allsubmittedviews}
    ...
    {foreach from=$allsubmittedviews item=view}
        <tr class="{cycle values='r0,r1'}">
          <td>
            <strong><a href="{$WWWROOT}{$view.url}">{$view.title|str_shorten_text:60:true}</a></strong>
            <div><a href="{$WWWROOT}{profile_url($view.user)}">{$view.sharedby}</a></div>
          </td>
          <td>
