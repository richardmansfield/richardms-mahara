{if $views}
  ...
  {foreach from=$views item=view}
    <tr class="{cycle values='r0,r1'}">
            <td><h4><a href="{$WWWROOT}{$view.url}">{$view.title}</a></h4>
              <div class="details">{$view.description|str_shorten_html:100:true|strip_tags|safe}</div></td>
            {if $view.sharedby}
            <td class="right">
                {if $view.group && $loggedin}
                  <a href="{$WWWROOT}{group_homepage_url($view.groupdata)}" class="s">{$view.sharedby}</a>
                {elseif $view.owner && $loggedin}
                  <a href="{$WWWROOT}{profile_url($view.user)}" class="s">{$view.sharedby}</a>
                {else}
                  {$view.sharedby}
                {/if}
