...
function user_statistics($limit, $offset, &$sitedata) {
    ...
    if ($maxfriends) {
        $data['strmaxfriends'] = get_string(
            ...
            'admin',
            round($meanfriends, 1),
            get_config('wwwroot') . profile_url($maxfriends),
    ...
    if ($maxviews) {
        $data['strmaxviews'] = get_string(
            ...
            get_config('wwwroot') . profile_url($maxviews),
    ...
    if ($maxgroups) {
        $data['strmaxgroups'] = get_string(
            ...
            get_config('wwwroot') . profile_url($maxgroups),
    ...
    $data['strmaxquotaused'] = get_string(
        ...
        get_config('wwwroot') . profile_url($maxquotaused),
    ...
    foreach ($viewdata as &$v) {
        ...
        if ($v->owner) {
            ...
            if ($v->group) {
                ...
                $v->ownerurl = 'institution/index.php?institution=' . $v->institution;
            }
        }
