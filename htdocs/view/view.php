...
if ($USER->is_logged_in() && $submittedgroup && group_user_can_assess_submitted_views($submittedgroup, $USER->get('id'))) {
    ...
    if ($view->get('submittedtime')) {
        $text = get_string('viewsubmittedtogroupon', 'view', get_config('wwwroot') . group_homepage_url($submittedgroup), hsc($submittedgroup->name), format_date(strtotime($view->get('submittedtime'))));
    }
    else {
        $text = get_string('viewsubmittedtogroup', 'view', get_config('wwwroot') . group_homepage_url($submittedgroup), hsc($submittedgroup->name));
    }
...
function releaseview_submit() {
    ...
    if ($submittedgroup) {
        ...
        redirect(get_config('wwwroot') . group_homepage_url($submittedgroup));
    }
    redirect($view->get_url());
}
