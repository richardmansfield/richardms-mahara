{foreach from=$views item=view}
    <tr class="{cycle values='r0,r1'}">
      <td class="sharedpages">
        <h3><a href="{$WWWROOT}{$view.url}">{$view.title|str_shorten_text:65:true}</a></h3>
        {if $view.sharedby}
        <span class="owner">
          {if $view.group}
      ...
      <td class="lastcomment s">
        {if $view.commenttext}
          ...
          {if $view.commentauthor}
            <a href="{$WWWROOT}{profile_url($view.commentauthor)}" class="poster">{$view.commentauthor|display_name}</a>
          {else}
