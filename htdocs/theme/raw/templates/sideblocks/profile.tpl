    <div class="sidebar-header">
      <div id="user-profileicon">
        <a href="{$WWWROOT}{$sbdata.url}"><img src="{profile_icon_url user=$sbdata.id maxheight=50 maxwidth=50}" alt=""></a>
      </div>
      <h3><a href="{$WWWROOT}{$sbdata.url}">{$sbdata.myname}</a></h3>
    </div>
...
{foreach from=$sbdata.groups item=group}
                    <li><a href="{$WWWROOT}{group_homepage_url($group)}">{$group->name}</a>{if $group->role == 'admin'} ({str tag=Admin section=group}){/if}</li>
{/foreach}
                </ul></li>
{/if}
{if $sbdata.views}
...
{foreach from=$sbdata.views item=view}
                    <li><a href="{$WWWROOT}{$view->url}">{$view->title}</a></li>
{/foreach}
                </ul>
            </li>
{/if}
