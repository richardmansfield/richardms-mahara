<h3><a href="{$WWWROOT}{group_homepage_url($group)}">{$group->name}</a></h3>
<h6>{foreach name=admins from=$group->admins item=user}<a href="{$WWWROOT}{profile_url($user)}">{$user|display_name}</a>{if !$.foreach.admins.last}, {/if}{/foreach}</h6>
